...
class TransformerEncoder(nn.Module):
    def __init__(
        self, vocab_size, d_model, num_heads, conv_hidden_dim, num_layers, dropout=0.1
    ):
    ...
    def forward(self, x, mask=None):
        ...
        positions = (
            torch.arange(0, seq_length).expand(x.shape[0], seq_length).to(x.device)
        )
...
TransformerEncoder(
  ...
  (position_embedding): Embedding(1000, 512)
  (layers): ModuleList(
...
input_tensor = torch.randint(
    0, vocab_size, (1, 20)
)  # batch size of 1 and sequence length of 20

input_tensor
# tensor([[4049, 3008,  685, 3446,  860, 4789, 4994,  193,  568,  896,  120,  883,
#          1328, 4449, 2094, 2173, 4155, 1946,  407, 1047]])


# Forward pass through the model
output = model(input_tensor, mask=None)
"""
tensor([[[ 1.2551,  0.5367,  0.3715,  ...,  0.5004, -0.3908,  1.0155],
         [ 0.0864, -0.5396,  1.0176,  ...,  1.3815, -0.7740,  1.0807],
         [ 2.3454, -0.9317, -0.8566,  ..., -2.6701,  0.0253, -0.7874],
         ...,
         [ 0.4298,  0.2210, -0.4724,  ..., -0.8902,  1.5278,  0.9213],
         [ 1.0809, -1.2135,  1.4987,  ..., -1.0632,  0.9613, -0.8514],
         [ 0.9051, -0.6801, -0.7058,  ...,  1.2860,  0.0609,  0.3629]]],
...
model.parameters()
# <generator object Module.parameters at 0x0000023EEEE63CA0>
...
input_tensor = torch.randint(
    0, vocab_size, (1, 5)
)  # batch size of 1 and sequence length of 5
# tensor([[4394,  981, 3808, 1241, 4258]])

# Forward pass through the model
embeddings = model(input_tensor, mask=None)
"""
tensor([[[-4.1446e-01, -1.3362e+00, -2.7922e-01,  ...,  5.7465e-01,
         [ 5.0447e-01, -2.9732e-02,  3.8075e-01,  ...,  7.4581e-01,
          -3.2834e-01,  9.0970e-01],
         [ 1.0643e+00,  2.1969e-02, -3.3705e-04,  ..., -1.9328e+00,
          -1.0562e+00, -9.2498e-01],
         [ 6.7705e-01, -1.4075e+00, -3.1317e-01,  ..., -1.5752e+00,
          -1.1852e+00, -6.9233e-01],
         [ 1.8980e+00,  9.7233e-01,  2.0635e-01,  ...,  4.4686e-01,
           2.0320e+00,  3.6587e-01]]], grad_fn=<NativeLayerNormBackward0>) """
...
def sentence_to_embeddings(sentence, model, word2id):
    ...
    input_tensor = torch.tensor(
        [word2id.get(word, word2id["[UNK]"]) for word in sentence.split()],
        dtype=torch.long,
    ).unsqueeze(0)
    embeddings = model(input_tensor, mask=None)
    return embeddings
...
print(
    f"The cosine similarity between the sentences '{sentence1}' and '{sentence2}' is {similarity_score:.2f}."
)
# The cosine similarity between the sentences 'the cat has a blue fish' and 'my sister's dog sleeps' is 0.56.
...
embeddings = np.array(
    [np.random.rand(512) for _ in words]
)  # 512 is the dimension of the embeddings
...
model_bert = BertModel.from_pretrained(model_name, cache_dir=".\\models")
...
print(
    f"The model has {count_parameters(new_model):,} trainable parameters, compared to the 21,986,304 of the original model"
)
...
similarity = "<FILL_IN>"
print(
    "Cosine similarity between original and scrambled sentence embeddings:",
    similarity.item(),
)
...
similarity = "<FILL_IN>"
print(
    f"Cosine similarity between embeddings of the word 'bank' in two different contexts: {similarity.item()}"
)
...
tokenizer = BertTokenizer.from_pretrained(
    "bert-base-uncased", cache_dir=DA.paths.datasets + "/models"
)
